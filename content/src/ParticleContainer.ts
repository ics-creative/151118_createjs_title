...
namespace project {
		...
		private enterFrameHandler(event:createjs.Event):void {
			...
			if(this._tickCount % 2 == 0){
				this._emitter.emit(
					...
					window.innerHeight / 5 * ( Math.random() - 0.5) + window.innerHeight * 6 / 10);
			}
			...
			if (this._isMouseMoved == true) {
				let pt = this.globalToLocal(this.getStage().mouseX, this.getStage().mouseY);
	...
	class Particle extends createjs.Shape {
		...
		constructor() {
			...
			const size = Math.random() * Math.random() * Math.random() * Math.random() * this.MAX_SIZE + 2;
			this.size = size;
		 ...
		 * @param emitY
		 */
		public resetParameters(emitX:number, emitY:number, startVx:number, startVy:number):void {
			this.x = emitX;
			this.y = emitY;
